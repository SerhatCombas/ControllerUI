"""ValidationReport: structured validation result (shared type).

Relocated from `features/SystemModelingModule/model/validation_report.py`
in the pre-S2.B.2 refactor: the type is now produced by multiple
features (workspace-graph rules via `GraphValidator`, configuration
rules via `ConfigurationValidator`), so it belongs at the
cross-feature shared layer per CLAUDE.md "features → shared/ types"
and ADR-018's "validationChanged payload is `ValidationReport`"
contract.

The original module path is preserved as a re-export shim
(`features/SystemModelingModule/model/validation_report.py`) so the
30+ existing imports inside `SystemModelingModule` continue to work
untouched. New code should import from this module directly. A
follow-up cleanup commit (S1.11 backlog) will migrate the shim
callers and drop the shim module.
...
* `decisions/ADR-018-signal-payload-contracts.md` (validationChanged payload)
"""
